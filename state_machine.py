...
def print_tape_transition(pos, state, transition):
    pos_copy = pos
    tape_copy = tape.copy()

    if (pos_copy < 0):
        tape_copy.insert(0, bcolors.BOLD + bcolors.UNDERLINE + bcolors.WARNING + machine['blank'] + bcolors.ENDC)
        pos_copy = 0
    elif pos_copy + 1 > len(tape):
        tape_copy.append(bcolors.BOLD + bcolors.UNDERLINE + bcolors.WARNING + machine['blank'] + bcolors.ENDC)
        pos_copy = 0
    else:
...
def run_state_machine(machine_input, machine_description):
    global tape, pos, machine
    machine = machine_description

    tape.extend(list(machine_input))
    ...
    while state not in machine['finals']:
        ...
        if transition == False:
            raise LookupError(f"No transition found for {read_tape()} in {state}")

        print_tape_transition(pos, state, transition)

        write_tape(transition['write'])
